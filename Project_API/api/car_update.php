...
try {
    ...
    if (isset($_FILES["main_image"]) && $_FILES["main_image"]["error"] === 0) {
        ...
        $stmt = $pdo->prepare("
            UPDATE cars SET title=?, color=?, year=?, price=?, phone=?, description=?, main_image=? WHERE id=?
        ");
        $stmt->execute([$title, $color, $year, $price, $phone, $description, $main_image_name, $car_id]);
    } else {
        $stmt = $pdo->prepare("
            UPDATE cars SET title=?, color=?, year=?, price=?, phone=?, description=? WHERE id=?
        ");
        $stmt->execute([$title, $color, $year, $price, $phone, $description, $car_id]);
    }
